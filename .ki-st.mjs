...
const st=await page.evaluate(()=>{
  const S=window.__ST
  if(!S) return 'none'
  return S.getAll().map(t=>({trigger:(t.trigger&&t.trigger.className||'').toString().slice(0,34), start:Math.round(t.start), end:Math.round(t.end), pinned:!!t.pin, anim:!!t.animation, dur:t.animation?+t.animation.duration().toFixed(2):null}))})
