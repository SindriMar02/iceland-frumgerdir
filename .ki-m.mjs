...
const spots=await page.evaluate(()=>{
  const y=e=>e?e.getBoundingClientRect().top+window.scrollY:null
  return {strata:y(document.querySelector('.ki-strata-static'))??y(document.querySelector('.ki-plx-deep--strata')),
          hs:y(document.querySelector('.ki-hs')), plate:y(document.querySelector('.is-plate')),
          hsEnd:(()=>{const e=document.querySelector('.ki-hs');return e?e.getBoundingClientRect().bottom+window.scrollY:null})()}})
...
await page.click('.ki-burger'); await new Promise(r=>setTimeout(r,240))
await page.screenshot({path:'/tmp/ki-gate/M-menu-mid.png'})
await new Promise(r=>setTimeout(r,700))
await page.screenshot({path:'/tmp/ki-gate/M-menu-open.png'})
