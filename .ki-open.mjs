...
const state=()=>({
  ...
  plates:[...document.querySelectorAll('[data-parallax-plate]')].map(e=>{const m=new DOMMatrix(getComputedStyle(e).transform);return Math.round(m.m42)}),
  room:(()=>{const e=document.querySelector('[data-parallax-layer="1"]');return e?new DOMMatrix(getComputedStyle(e).transform).a.toFixed(3):null})(),
  corner:getComputedStyle(document.querySelector('.parallax__corner')).opacity,
  ov:document.documentElement.style.overflow, sy:window.scrollY,
  tl:!!document.querySelector('.gsap-marker-start')||undefined,
})
...
for (const [w,h,tag] of [[1600,1000,'desktop'],[390,844,'phone']]) {
  ...
  console.log(`${tag} FIRST PAINT (no JS): plates y ${s0.plates.join(',')} · corner ${s0.corner} · pixel at name ${covered} (stone if <60) · frame mean ${mean}`)
  ...
  for (const at of [100,500,900,1300,1700,2100,2500,2900,3400,4000]) {
    ...
    const s=await p2.evaluate(state); rows.push(`  ${String(at).padStart(4)}ms  plates ${s.plates.map(v=>String(v).padStart(5)).join(' ')}  room ${s.room}  corner ${s.corner}  ov "${s.ov}"  sy ${s.sy}`)
  }
  ...
  console.log(`${tag} after intro, scrollTo(600): sy ${after.sy} plates ${after.plates.join(',')} (scrub live if plates moved)`)
  ...
  console.log(`${tag} REPEAT VISIT at 200ms: plates ${rep.plates.join(',')} corner ${rep.corner} seen=${await p2.evaluate(()=>document.documentElement.dataset.kiSeen)}`)
  await ctx.close()
}
...
const rm=await p3.evaluate(state); console.log(`reduced motion at 300ms: plates ${rm.plates.join(',')} corner ${rm.corner} ov "${rm.ov}"`)
