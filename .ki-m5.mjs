...
for (const [name,y] of Object.entries(spots)) {
  ...
  const st=await page.evaluate(()=>{const d=document.querySelector('.ki-plx-deep--strata');const r=d.getBoundingClientRect();return 'deep top '+Math.round(r.top)+' bottom '+Math.round(r.bottom)+' h '+Math.round(r.height)+' vh '+window.innerHeight+' overflow '+(r.top<0||r.bottom>window.innerHeight)});console.log(name, st)
}
