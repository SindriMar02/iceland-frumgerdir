...
interface Props {
  /** her works have no titles; the label is composed from the exhibition + ordinal */
  nameFor: (w: Work) => string
  /** The reference's list layout gives one work a screen, with the odometer rolling
   *  its name — right for a six-work show, punishing for a forty-one-work one
   *  (measured: 36,506px of scroll against 4,086 in grid). Big bodies open as a grid. */
...
export function SeriesWorks({ works, grid, full, onOpen, lbl, listLabel, gridLabel, nameFor, initialLayout = 'list' }: Props) {
  const [layout, setLayout] = useState<'list' | 'grid'>(initialLayout)
  /* the odometer only means something once a painting is under it */
  const [odoOn, setOdoOn] = useState(false)
  useEffect(() => {
    const onScroll = () => setOdoOn(window.scrollY > window.innerHeight * 0.5)
    onScroll()
            ...
            >
              ...
              {layout === 'grid' && (
                <span className={`${LBL} mt-3 flex items-baseline gap-2 text-black`}>
                  ...
                  <span className="truncate">{nameFor(w)}</span>
                </span>
              )}
            </button>
          </li>
