...
export interface Slide {
  id: string
  hero: string
  /** true when the source is too small to bleed: the painting is hung whole on the
        ...
        >
          ...
          {s.variants.map((v) => (
            <img
              ...
              className={`absolute inset-0 h-full w-full will-change-[clip-path] ${s.contain ? 'object-contain p-6 md:p-16' : 'object-cover'}`}
            />
          ))}
          {!s.contain && (
            <div aria-hidden className="sossa-topveil absolute inset-x-0 top-0 h-[150px]" />
          )}
        </div>
      ))}
