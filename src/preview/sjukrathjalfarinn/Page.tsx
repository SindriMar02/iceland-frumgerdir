...
import { DragRow, Plate, Rise, SplitHeading, useScrollEngine } from './primitives'
import { Preloader } from './Preloader'
import { Quiz } from './Quiz'
import { Marquee } from './Marquee'
import { RosterMoment, RosterDirectory } from './Roster'
...
export default function SjukrathjalfarinnPage() {
  ...
  useEffect(() => setThemeColor('#E8E7E2'), [])
  ...
  return (
    <div style={{ background: '#E8E7E2', color: INK, fontFamily: BODY, minHeight: '100vh' }}>
      <style>{`
        @font-face { font-family:'SJR Display'; src:url('${import.meta.env.BASE_URL}sjukrathjalfarinn/fonts/Anton-Regular.woff2') format('woff2'); font-weight:400 900; font-style:normal; font-display:swap; }
        @font-face { font-family:'SJR General'; src:url('${import.meta.env.BASE_URL}sjukrathjalfarinn/fonts/GeneralSans-Regular.woff2') format('woff2'); font-weight:400; font-style:normal; font-display:swap; }
        @font-face { font-family:'SJR General'; src:url('${import.meta.env.BASE_URL}sjukrathjalfarinn/fonts/GeneralSans-Medium.woff2') format('woff2'); font-weight:500; font-style:normal; font-display:swap; }
        @font-face { font-family:'SJR General'; src:url('${import.meta.env.BASE_URL}sjukrathjalfarinn/fonts/GeneralSans-Semibold.woff2') format('woff2'); font-weight:600; font-style:normal; font-display:swap; }
        @font-face { font-family:'SJR Plex Mono'; src:url('${import.meta.env.BASE_URL}sjukrathjalfarinn/fonts/IBMPlexMono-Regular.woff2') format('woff2'); font-weight:400; font-style:normal; font-display:swap; }
        @font-face { font-family:'SJR Plex Mono'; src:url('${import.meta.env.BASE_URL}sjukrathjalfarinn/fonts/IBMPlexMono-SemiBold.woff2') format('woff2'); font-weight:600; font-style:normal; font-display:swap; }
      `}</style>
      ...
      <header
        ...
        style={{ background: 'rgba(232,231,226,.92)', backdropFilter: 'blur(10px)', borderBottom: `1px solid ${HAIR}` }}
      >
        <div className="mx-auto flex max-w-[1180px] items-center gap-3 px-5 py-3 sm:px-8" style={{ minHeight: 68 }}>
          <a href="#efst" className={`flex items-center ${FOCUS}`} style={{ minHeight: 44 }} aria-label="Sjúkraþjálfarinn, efst á síðu">
            <img src={IMG.logo} alt="Sjúkraþjálfarinn" style={{ height: 26, width: 'auto', display: 'block' }} />
          </a>
          ...
          <a
            href={PHONE_HREF}
            className={`ml-auto inline-flex shrink-0 items-center gap-1.5 rounded-[.375rem] px-3 text-[.84rem] sm:gap-2 sm:px-5 sm:text-[.92rem] lg:ml-0 ${FOCUS}`}
        ...
        {menu && (
          <div className="border-t lg:hidden" style={{ borderColor: HAIR, background: '#E8E7E2' }}>
      ...
      <main id="efst">
        {/* ── 1 · HERO = QUIZ (D2) + mouse-parallax layers (D6) — bone ── */}
          ...
          <div className="grid gap-10 lg:grid-cols-[1.06fr_.94fr] lg:items-start lg:gap-14">
            <div>
              <p style={{ fontFamily: MONO, fontSize: '.76rem', letterSpacing: '.13em', color: MUTE }} className="uppercase">
                Sjúkraþjálfun í Hafnarfirði síðan {FOUNDED}
              </p>
              <h1
                className="mt-4"
                style={{ fontFamily: DISPLAY, fontWeight: 800, fontSize: 'clamp(2.5rem, 6.4vw, 4.3rem)', letterSpacing: '-.02em', lineHeight: 1.08, color: INK }}
              >
                Hvar finnur þú til?
              </h1>
              <p className="mt-5 max-w-[46ch]" style={{ fontSize: 'clamp(1.02rem, 1.8vw, 1.2rem)', lineHeight: 1.55, color: SOFT }}>
                Svaraðu tveimur stuttum skrefum og fáðu ábendingu um hvaða þjónusta hjá okkur hentar þér og hvaða
                sjúkraþjálfari sinnir því, með beinni leið til að hringja eða senda fyrirspurn.
              </p>
        ...
        {/* ── 2 · MARQUEE (D4) — deep pine band ─────────────────────────── */}
        <section style={{ background: DEEP }}>
          <div className="py-12 sm:py-16">
            <Marquee items={MARQUEE_ITEMS} tone="onDeep" />
          </div>
        </section>

        {/* ── 3 · TRUST: tvö hús síðan 1984 — bone ──────────────────────── */}
          ...
          <div className="mx-auto max-w-[1180px] px-5 py-20 sm:px-8 sm:py-28">
            <div className="grid gap-8 lg:grid-cols-[1.3fr_.7fr] lg:items-start lg:gap-14">
              <div>
                ...
                <SplitHeading as="h2" id="ar-h" className="sjr-h2 mt-3">
                  Tvö hús, sama fólkið síðan 1984
                </SplitHeading>

                <Rise delay={1}>
                  <p className="mt-6 max-w-[62ch]" style={{ color: SOFT, lineHeight: 1.65, fontSize: '1.02rem' }}>
                    Sjúkraþjálfarinn hefur verið starfrækt í Hafnarfirði síðan 1984. Í dag reka þau tvær stöðvar,
                    Strandgötu 75 og Bæjarhraun 2, með {STAFF.length} nafngreindum sjúkraþjálfurum samtals. Hvor stöð
                    hefur sinn eigin æfingasal og sitt eigið andlit við afgreiðsluna.
                  </p>
                </Rise>
              </div>

              <Plate src={IMG.clinic} alt="Sjúkraþjálfarinn, mynd af húsnæðinu af vef þeirra." ratio="4 / 5" className="rounded-[.5rem]" parallax />
            </div>

            <div className="mt-11 grid gap-6 md:grid-cols-2">
              {HOUSES.map((house, i) => (
                <Rise key={house.id} delay={i} as="article">
                  <div className="overflow-hidden rounded-[.625rem]" style={{ border: `1px solid ${HAIR}` }}>
        ...
        {/* ── 4 · ÞJÓNUSTA — bone ───────────────────────────────────────── */}
        <section id="thjonusta" style={{ background: '#E8E7E2' }} aria-labelledby="thjonusta-h">
          <div className="mx-auto max-w-[1180px] px-5 py-20 sm:px-8 sm:py-28">
              ...
              {SERVICES.map((s, i) => {
                ...
                return (
                  <Rise key={s.id} delay={i % 3} as="article" className={wide ? 'sm:col-span-2' : ''}>
                    <div
                      className="flex h-full flex-col rounded-[.5rem] p-7 sm:p-8"
        ...
        {/* ── 5 · HÓPAR — bone ───────────────────────────────────────────── */}
        <section id="hopar" style={{ background: '#E8E7E2' }} aria-labelledby="hopar-h">
          <div className="mx-auto max-w-[1180px] px-5 py-20 sm:px-8 sm:py-28">
            ...
            <Rise>
              <p className="mt-4 max-w-[58ch]" style={{ color: SOFT, fontSize: '1.05rem', lineHeight: 1.6 }}>
                Fastir hópatímar með sjúkraþjálfara, á föstum tíma í hverri viku. Tveir þeirra fá sinn eigin stað hér,
                því fáar stöðvar í bænum bjóða upp á þá.
              </p>
            </Rise>
            ...
            <div className="mt-11 grid gap-6 lg:grid-cols-2">
              {featuredHopar.map((h, i) => (
                <Rise key={h.id} delay={i} as="article">
                  <div className="overflow-hidden rounded-[.625rem]" style={{ border: `1px solid ${HAIR}`, background: '#fff' }}>
                    <Plate
            ...
            <DragRow className="mt-5">
              {otherHopar.map((h) => (
                <div
                  key={h.id}
                  className="w-[19rem] rounded-[.5rem] p-6"
        ...
        {/* ── 6 · ROSTER MOMENT (D3, part A) — ACCENT PUNCH #1 ──────────── */}
        <RosterMoment />

        {/* ── 7 · ROSTER DIRECTORY (D3, part B) — deep pine ─────────────── */}
        <RosterDirectory />

        {/* ── 8 · PRACTICAL: greiðsla, staðsetning, afboðun — bone ──────── */}
        <section id="stadsetning" style={{ background: '#E8E7E2' }} aria-labelledby="stad-h">
          <div className="mx-auto max-w-[1180px] px-5 py-20 sm:px-8 sm:py-28">
            <div className="grid gap-10 lg:grid-cols-[.9fr_1.1fr] lg:items-start lg:gap-16">
              <Plate src={IMG.clinicKnee} alt="Sjúkraþjálfari skoðar hné skjólstæðings á bekk." ratio="4 / 5" className="rounded-[.5rem]" parallax />
              <div>
                <SplitHeading as="h2" id="stad-h" className="sjr-h2">
                  Hvernig borgunin virkar
                </SplitHeading>
                <Rise>
                  <p className="mt-4 max-w-[52ch]" style={{ color: SOFT, fontSize: '1.05rem', lineHeight: 1.6 }}>
                    Engin opinber verðskrá er birt á vefnum þeirra í dag. Þetta er það sem liggur fyrir um
                    niðurgreiðslu og greiðslufyrirkomulag.
                  </p>
                </Rise>
                ...
                <div className="mt-8 grid gap-4 sm:grid-cols-2">
                  {INSURANCE_CARDS.slice(0, 2).map((c, i) => (
                    <Rise key={c.head} delay={i} as="article">
                      <div className="h-full rounded-[.5rem] p-6" style={{ background: MIST, border: `1px solid ${HAIR}` }}>
                ...
                <Rise delay={2} className="mt-4">
                  <div className="rounded-[.5rem] p-6" style={{ background: ACCENT_DEEP, color: '#fff' }}>
            ...
            <div className="mt-16 grid gap-6 md:grid-cols-2">
              {HOUSES.map((house, i) => (
                <Rise key={house.id} delay={i} as="article">
                  <div className="h-full rounded-[.5rem] p-7 sm:p-8" style={{ background: '#fff', border: `1px solid ${HAIR}` }}>
            ...
            <Rise delay={2} className="mt-8">
              <div className="rounded-[.5rem] p-6" style={{ background: '#fff', border: `1px solid ${HAIR}` }}>
        ...
        {/* ── 9 · FAQ (D7) — bone ────────────────────────────────────────── */}
        <section style={{ background: '#fff' }} aria-labelledby="faq-h">
          <div className="mx-auto max-w-[1180px] px-5 py-20 sm:px-8 sm:py-28">
            <SplitHeading as="h2" id="faq-h" className="sjr-h2">
              Algengar spurningar
            </SplitHeading>
            <Rise className="mt-11">
              <FAQAccordion />
            </Rise>
          </div>
        </section>

        {/* ── 10 · BOOKING CTA — ACCENT PUNCH #2 ────────────────────────── */}
        <section aria-labelledby="cta-h" style={{ background: ACCENT }}>
          <div className="mx-auto max-w-[1180px] px-5 py-24 text-center sm:px-8 sm:py-32">
            <Rise>
              <p style={{ fontFamily: MONO, fontSize: '.78rem', letterSpacing: '.12em', color: INK }} className="uppercase opacity-70">
                {STAFF.length} sjúkraþjálfarar · tvær stöðvar · {YEARS_RUNNING} ár
              </p>
              <h2
                id="cta-h"
                className="mx-auto mt-5 max-w-[20ch]"
                style={{ fontFamily: DISPLAY, fontWeight: 800, fontSize: 'clamp(2.1rem, 6vw, 3.8rem)', letterSpacing: '-.02em', lineHeight: 1.1, color: INK }}
              >
              ...
              <div className="mt-9 flex flex-wrap items-center justify-center gap-3">
                <a
                  ...
                  style={{ background: INK, color: '#fff', fontFamily: DISPLAY, fontWeight: 700, fontSize: 'clamp(1.05rem, 2.6vw, 1.3rem)', padding: '18px 32px', minHeight: 58 }}
                >
                  Taka spurningarnar
                </a>
                <a
                  href={PHONE_HREF}
                  className={`inline-flex items-center rounded-[.375rem] ${FOCUS}`}
                  style={{ border: `1.5px solid ${INK}`, color: INK, fontFamily: DISPLAY, fontWeight: 600, fontSize: '1.05rem', padding: '17px 28px', minHeight: 58 }}
                >
                  {PHONE_DISPLAY}
                </a>
              </div>
        ...
        {/* ── 11 · SIGN-OFF — near-black ─────────────────────────────────── */}
        <section aria-labelledby="loka-h" style={{ background: FOOTER }}>
          <div className="mx-auto max-w-[1180px] px-5 py-20 text-center sm:px-8 sm:py-24">
            <Rise>
              <img src={IMG.logo} alt="Sjúkraþjálfarinn" className="mx-auto" style={{ height: 30, width: 'auto', filter: 'brightness(0) invert(1)', opacity: 0.92 }} />
              <h3 id="loka-h" className="sr-only">
                Hafa samband
              </h3>
              <p className="mt-8" style={{ fontFamily: MONO, fontSize: '.82rem', color: DEEP_MUTE, lineHeight: 1.7 }}>
                {LEGAL_NAME} · Strandgata 75 og Bæjarhraun 2, 220 Hafnarfjörður
                <br />
                <a href={PHONE_HREF} className={`sjr-link inline-flex items-center ${FOCUS_DEEP}`} style={{ color: CREAM, minHeight: 44 }}>
                  {PHONE_DISPLAY}
                </a>{' '}
                ·{' '}
                <a href={EMAIL_HREF} className={`sjr-link inline-flex items-center ${FOCUS_DEEP}`} style={{ color: CREAM, minHeight: 44 }}>
                  {EMAIL}
                </a>
              </p>
            </Rise>
      ...
      {/* mobile-only slim fixed phone strip — the number is reachable at
          every scroll position (desktop already has it in the sticky header) */}
      <a href={PHONE_HREF} className="sjr-phone-strip lg:hidden" aria-label={`Hringja í ${PHONE_DISPLAY}`}>
