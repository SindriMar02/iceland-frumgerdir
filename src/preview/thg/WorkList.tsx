...
import { IMG, PROJECTS, KIND_LABEL } from './data'
...
/* ═════════════════════════════════════════════════════════════════════════
   ...
   re-prefixed thg-. Copied rather than imported: repo law is zero code bleed
      ...
      <ul className="thg-wlist">
        {PROJECTS.map((p, i) => (
          <li key={p.key}>
            ...
            >
              {!fine ? (
                <span className="thg-wthumb">
                  <img src={IMG(p.image)} alt={p.alt} loading="lazy" decoding="async" />
                </span>
              ) : null}
              <span className="thg-wname">{p.name}</span>
              ...
              <span className="thg-wmeta">
                {[p.place, p.year, p.size].filter(Boolean).join(' · ')}
              </span>
              <span className="thg-wstate">{KIND_LABEL[p.kind]}</span>
            </a>
          </li>
        ))}
      </ul>
...
.thg-preview{position:absolute;left:0;top:0;z-index:30;pointer-events:none;width:300px;height:200px;
  overflow:hidden;background:${RULE};transition:opacity .34s cubic-bezier(.17,.84,.44,1),scale .34s cubic-bezier(.17,.84,.44,1)}
.thg-preview img{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;
  transition:opacity .45s cubic-bezier(.17,.84,.44,1),transform .6s cubic-bezier(.17,.84,.44,1),filter .45s cubic-bezier(.17,.84,.44,1)}

.thg-wlist{list-style:none;margin:0;padding:0;border-top:1px solid ${RULE}}
.thg-wrow{display:grid;grid-template-columns:1fr auto;gap:.4rem 1.5rem;align-items:baseline;
...
@media (hover:hover) and (pointer:fine){.thg-wrow:hover{padding-left:.9rem}}
...
.thg-wname{font-size:clamp(1.3rem,3.3vw,2.7rem);letter-spacing:-.026em;line-height:1.04;grid-column:1}
.thg-wmeta{grid-column:1;color:${MUTED};font-size:clamp(.86rem,1.05vw,1rem)}
.thg-wstate{grid-column:2;grid-row:1/3;text-align:right;color:${MUTED};font-size:clamp(.9rem,1.15vw,1.06rem);white-space:nowrap}
.thg-wrow:focus-visible{outline:2px solid ${INK};outline-offset:4px}

/* coarse pointer / reduced motion: every row carries its own image */
.thg-wthumb{grid-column:1;grid-row:1;display:block;position:relative;width:100%;aspect-ratio:3/2;
