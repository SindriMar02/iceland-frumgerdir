...
import { COLLECTIONS, EXHIBITIONS } from './data'
...
const GRID = (id: string) => `${BASE}sbb/grid/${id}.jpg`
const FULL = (id: string) => `${BASE}sbb/work/${id}.jpg`
const HERO = (id: string) => `${BASE}sbb/hero/${id}.jpg`
...
const yearRange = (ws: Work[]) => {
  // some of his items carry no year; the band shows the range of those that do
  const ys = ws.map((w) => w.year).filter((y): y is number => typeof y === 'number')
  if (!ys.length) return ''
...
/* Hero crop, MEASURED not guessed: each position is the one where the strip the
 * MetaBand sits in (42–58% of the viewport height) is darkest, because the band
 * text is always white. Generated by scratchpad/hero-ink measurement, 2026-08-06. */
type Ink = 'light' | 'dark'
const HERO_INK: Record<string, { band: Ink; pos: string }> = {
  'staumvotn-og-vatnsfletir': { band: 'dark', pos: '50% 70%' },
  hedinsfjordur: { band: 'light', pos: '50% 70%' },
  'modan-graa': { band: 'dark', pos: '50% 70%' },
  merlmyndir: { band: 'light', pos: '50% 30%' },
  bruara: { band: 'dark', pos: '50% 70%' },
  '7-himnar': { band: 'dark', pos: '50% 60%' },
  'litir-kvarans': { band: 'dark', pos: '50% 30%' },
  vatnslitur: { band: 'light', pos: '50% 70%' },
  '360-dagar-i-grasagardinum': { band: 'dark', pos: '50% 50%' },
  vindvaettir: { band: 'dark', pos: '50% 70%' },
  'i-minningu-rothko': { band: 'dark', pos: '50% 70%' },
  'sjotta-svitan': { band: 'dark', pos: '50% 70%' },
  'stanmer-skogur': { band: 'dark', pos: '50% 30%' },
  'giardini-publicchi-treemix-remix': { band: 'dark', pos: '50% 70%' },
  'gomul-verk-skolaverk': { band: 'light', pos: '50% 30%' },
}
...
/* The four heroes whose band strip stays above ~150 luminance even at its best
 * crop — they need the grain to keep white type readable. Same measurement. */
const GRAINED = ['gomul-verk-skolaverk', 'hedinsfjordur', 'merlmyndir', 'vatnslitur']
  ...
  return (
    <div className="pointer-events-none absolute inset-x-0 top-1/2 z-20 -translate-y-1/2">
      <div className="relative flex flex-col items-start gap-3 px-6 text-white md:flex-row md:items-center md:gap-0 md:px-10">
        <span className="sbb-display whitespace-nowrap text-[13px] font-medium tracking-[0.1em] tabular-nums">
          {years}
        </span>
        <span className="sbb-display text-[clamp(34px,4.6vw,64px)] uppercase leading-[0.95] tracking-[-0.01em] md:ml-[8vw]">
          {name}
        </span>
        <span className="sbb-display flex items-center gap-3 whitespace-nowrap text-[13px] font-medium tracking-[0.1em] tabular-nums md:ml-auto">
          {pad2(count)}
          <Rule />
          {label}
        </span>
...
function IndexGrid({
  ...
  return (
    <ul
      ref={ref}
      className="sbb-grid mx-auto grid list-none grid-cols-2 justify-center gap-x-[26px] gap-y-[44px] px-6
          ...
          >
            <span className="block aspect-square w-full overflow-hidden">
              <img
                ...
                width={w.w}
                height={w.h}
                className="sbb-thumb block h-full w-full object-contain object-bottom will-change-transform"
              />
            </span>
...
/* ------------------------------------------------------------------ *
 * DEVICE — SÝNINGAR. He has no webshop, so the slot the reference gives to
 * commerce goes to the thing he actually has: 35 solo exhibitions since 1991
 * and fourteen public collections. Both lists are verbatim from his own CV
 * page. This is what replaces a price on a painter of his standing.
 * ------------------------------------------------------------------ */
function Exhibitions({ showLabel, collLabel }: { showLabel: string; collLabel: string }) {
  const ref = useRef<HTMLDivElement>(null)

  /* This list drives its own reveal instead of going through the shared
   * useRevealOnLoader cue. The cue fires two frames after the view switches,
   * which can land BEFORE this component's effect has built its timeline — the
   * play() is then missed and the rows sit frozen at the start state forever.
   * Measured: rows natural at 0.6s, start state written at 2.0s, still frozen
   * at 6.0s. An unpaused fromTo cannot get stranded that way.
   * fromTo, never from: React StrictMode remounts would otherwise leave the
   * second timeline reading autoAlpha:0 as its END value. */
  useEffect(() => {
    const el = ref.current
    if (!el || reduceMotion()) return
    const rows = [...el.querySelectorAll<HTMLElement>('[data-ex-row]')]
    if (!rows.length) return
    const tw = gsap.fromTo(
      rows,
      { yPercent: LIST_REVEAL.yPercent, autoAlpha: 0 },
      {
        yPercent: 0,
        autoAlpha: 1,
        duration: LIST_REVEAL.duration,
        // bound the TOTAL stagger: 40 rows at the reference's 0.1s each would be
        // a four second wait before his exhibition list becomes readable
        stagger: { amount: 0.9 },
  ...
  return (
    <div ref={ref} className="mx-auto max-w-[1400px]">
      <p className={`${LBL} text-black/45`}>{showLabel}</p>
      <ul className="mt-8 list-none border-t border-black/10">
        {EXHIBITIONS.map((e, i) => (
          <li
            key={`${e.year}-${i}`}
            data-ex-row=""
            className="grid grid-cols-[3.4rem_1fr] items-baseline gap-x-4 border-b border-black/10 py-5
                       md:grid-cols-[5rem_minmax(0,1.1fr)_minmax(0,1fr)] md:gap-x-8"
          >
            <span className={`${LBL} tabular-nums text-black/45`}>{e.year}</span>
            <span className="sbb-display text-[clamp(17px,1.9vw,26px)] leading-[1.15] tracking-[-0.01em]">
              {e.title}
            </span>
            <span className={`${LBL} col-start-2 mt-1 normal-case tracking-[0.04em] text-black/45 md:col-start-3 md:mt-0`}>
              {e.venue}
            </span>
          </li>
        ))}
      ...
      <p className={`${LBL} mt-20 text-black/45`}>{collLabel}</p>
      <ul className="mt-8 grid list-none grid-cols-1 gap-x-10 gap-y-3 sm:grid-cols-2 lg:grid-cols-3">
        {COLLECTIONS.map((c) => (
          <li key={c} data-ex-row="" className="border-b border-black/10 py-3">
            <span className={`${LBL} normal-case tracking-[0.04em]`}>{c}</span>
          </li>
        ))}
      </ul>
    </div>
...
export default function SigtryggurBjarniPage() {
  const company = getPreviewCompany('sigtryggur')
  ...
  useEffect(() => {
    document.title = 'Sigtryggur Bjarni Baldvinsson'
  }, [])
  ...
  return (
    <div lang={L ? 'is' : 'en'} className="min-h-screen bg-[#FAFAFA] text-black antialiased">
      ...
      <style>{`
        ...
        .sbb-cell:hover .sbb-thumb{opacity:.55}
        .sbb-topveil{background:linear-gradient(to bottom,rgba(0,0,0,.46),rgba(0,0,0,.20) 38%,rgba(0,0,0,0) 100%);pointer-events:none;z-index:10}
        .sbb-grain{
        ...
        @media (prefers-reduced-motion: reduce){.sbb-thumb{transition:none}}
      `}</style>
      ...
      >
        ...
        <button
          type="button"
          onClick={() => nav({ k: 'home' })}
          className="sbb-display text-[20px] uppercase leading-none tracking-[0.01em] transition-opacity hover:opacity-50 md:text-[26px]"
        >
          Sigtryggur Bjarni
        </button>
        <nav className="ml-auto hidden items-center gap-8 lg:flex" aria-label={L ? 'Aðalvalmynd' : 'Main'}>
          <span className="flex items-center gap-5">
            ...
            <button type="button" className={navItem} onClick={() => nav({ k: 'syningar' })}>
              {tr('exhibitions')}
            </button>
          </span>
          <span className="flex items-center gap-5">
            ...
            <button type="button" className={navItem} onClick={() => nav({ k: 'medium', m: 'olia' })}>
              {tr('canvas')}
            </button>
            <button type="button" className={navItem} onClick={() => nav({ k: 'medium', m: 'vatnslitur' })}>
              {tr('paper')}
            </button>
          </span>
          <span className="flex items-center gap-5">
      ...
      <main key={JSON.stringify(view)}>
        {view.k === 'home' && (
          <>
            <h1 className="sr-only">Sigtryggur Bjarni Baldvinsson {L ? '· málverk og vatnslitamyndir' : '· paintings and watercolours'}</h1>
          ...
          return (
            <>
              <section data-hero-panel="" className="relative h-[100svh] w-full overflow-hidden bg-black">
                ...
                />
                {GRAINED.includes(s.id) && <div aria-hidden className="sbb-grain absolute inset-0" />}
              ...
              {/* The device this whole build turns on: his series are not a
                  curation, they are his exhibitions. Where a series was shown as
                  a solo show, the show is named here straight from his own CV. */}
              {s.show && (
                <div className="px-6 pt-16 md:px-10">
                  <div className="mx-auto flex max-w-[1400px] flex-wrap items-baseline gap-x-5 gap-y-2 border-t border-black/10 pt-6">
                    <span className={`${LBL} text-black/40`}>{tr('shownAt')}</span>
                    <span className="sbb-display text-[clamp(16px,1.7vw,22px)] leading-none tracking-[-0.01em]">
                      {s.show.title}
                    </span>
                    <span className={`${LBL} normal-case tracking-[0.04em] text-black/45`}>
                      {s.show.venue}
                    </span>
                    <span className={`${LBL} ml-auto tabular-nums text-black/45`}>{s.show.year}</span>
                  </div>
                </div>
              )}
              <div className="pt-16">
              ...
              <div className="mx-auto grid max-w-[1400px] gap-12 lg:grid-cols-[1fr_260px] lg:gap-24">
                <div ref={workImgRef}>
                  <div className="overflow-hidden">
                    <img
                      src={FULL(w.id)}
                      alt={w.tech ? `${w.title}, ${w.tech}` : w.title}
                      loading="eager"
                      decoding="async"
                      width={w.w}
                      height={w.h}
                ...
                <aside ref={workTextRef} className="lg:sticky lg:top-[110px] lg:self-start">
                  ...
                  <h1 className="mt-5 sbb-display text-[26px] uppercase leading-none tracking-[0em]" data-reveal-text="">
                    {w.title}
                  </h1>
                  <dl className="mt-10 space-y-4">
                    {([
                      [tr('year'), w.year ? String(w.year) : null],
                      [tr('size'), w.size],
                      [tr('technique'), w.tech],
                      [tr('owner'), w.prov],
        ...
        {view.k === 'medium' && (() => {
          ...
          const heading = view.m === 'allt' ? tr('all') : view.m === 'olia' ? tr('canvas') : tr('paper')
          return (
            <div className="px-6 pb-24 pt-[110px] md:px-10">
              <div className="mx-auto flex max-w-[1400px] items-baseline gap-6 border-b border-black/10 pb-6">
                <h1 className="sbb-display text-[26px] uppercase leading-none tracking-[0em]">{heading}</h1>
        ...
        {view.k === 'syningar' && (
          <div className="px-6 pb-24 pt-[110px] md:px-10">
            <h1 className="sr-only">{tr('exhibitions')}</h1>
            <div className="mx-auto max-w-[1400px]">
              <p className={`${LBL} max-w-[54ch] normal-case leading-[1.9] tracking-[0.04em] text-black/50`}>
                {tr('exhibitionsIntro')}
              </p>
            </div>
            <div className="pt-16">
              <Exhibitions showLabel={tr('soloShows')} collLabel={tr('inCollections')} />
            </div>
          </div>
        )}
        ...
        {view.k === 'about' && (
          <div ref={aboutRef} className="px-6 pb-24 pt-[110px] md:px-10">
            <div className="mx-auto grid max-w-[1400px] gap-12 lg:grid-cols-[1fr_1fr] lg:gap-24">
              <div>
                <h1 className="sbb-display text-[26px] uppercase leading-none tracking-[0em]" data-reveal-text="">
                  {tr('aboutTitle')}
                </h1>
          ...
          <div ref={aboutRef} className="px-6 pb-24 pt-[110px] md:px-10">
            <div className="mx-auto max-w-[1400px]">
              ...
              {/* The enquiry basket is the intended route, but a contact page that
                  ...
                  end. His own published address, from his contact page. */}
              <p className={`${LBL} mt-12 block text-black/45`}>{tr('orWriteDirect')}</p>
              <a
                href={`mailto:${company.ownerEmail}`}
                className="sbb-display mt-2 inline-block text-[clamp(20px,4.4vw,30px)] lowercase leading-none tracking-[0em] transition-opacity hover:opacity-45"
              >
                {company.ownerEmail}
              </a>
            </div>
      ...
      {menu && (
        ...
        >
          <div className="sticky top-0 z-10 flex h-[58px] items-center bg-[#FAFAFA] px-6 md:px-10">
            <span className="sbb-display text-[20px] uppercase leading-none tracking-[0.01em] md:text-[26px]">Sigtryggur Bjarni</span>
          ...
          <nav className="flex flex-wrap gap-x-6 gap-y-4 px-6 pb-12 pt-6 md:px-10" aria-label={tr('menu')}>
            {[
              ...
              [tr('exhibitions'), () => nav({ k: 'syningar' })],
              [tr('canvas'), () => nav({ k: 'medium', m: 'olia' })],
              [tr('paper'), () => nav({ k: 'medium', m: 'vatnslitur' })],
