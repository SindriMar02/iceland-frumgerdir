...
import { AUDIENCES, GROUPS, SPURDU, allFor, otherAudiences, type Audience } from './spurdu-data'
...
export function QuestionIndex({ compact = false }: { compact?: boolean }) {
  ...
  /* compact (service pages) shows a short set; the landing carries the lot */
  const items = compact ? allFor(aud).slice(0, 5) : allFor(aud)

  /*
   * The answer opens HERE, under the question. An earlier version linked each
   * question through to a separate page that carried a search field, which
   * turned a FAQ into a destination that looked like a chatbot. This is meant
   * to be a smart FAQ and nothing more: pick who you are, read the answer that
   * applies to you, in place.
   */
  return (
    <div>
          ...
          {AUDIENCES.map((a) => {
            ...
            return (
              <button
                ...
                onClick={() => {
                  setAud(a.id)
                  /* Keep the same question open across a switch when the other
                     audience has an answer to it. Watching the answer change is
                     the entire point of splitting them. */
                  if (openId && !GROUPS.find((g) => g.id === openId)?.variants[a.id]) setOpenId(null)
                }}
      ...
      <ul className="mt-6 space-y-2.5">
        {items.map(({ group, variant }) => {
          ...
          const panelId = `faq-${aud}-${group.id}`
          return (
            <li
              key={group.id}
              className="overflow-hidden rounded-[16px]"
              style={{ background: '#fff', boxShadow: `inset 0 0 0 1px ${open ? C.clay : C.line}` }}
            >
              <h3>
                <button
                  type="button"
                  ...
                  className="bofs-focus flex w-full items-start gap-3 px-4 py-3.5 text-left text-[15.5px] font-semibold leading-snug"
                  style={{ color: C.cocoa }}
                >
              ...
              {open && (
                <div id={panelId} className="px-4 pb-5">
                  <p className="bofs-pretty max-w-[68ch] text-[15px] leading-relaxed" style={{ color: C.body }}>
                    {pick(variant.a)}
                  </p>
                  {group.emergency && (
                    <a
                      href="tel:112"
                      className="bofs-focus mt-4 inline-flex items-center gap-2 rounded-full px-5 py-2.5 text-[15px] font-bold"
                  ...
                  {group.source && (
                    <p className="mt-3 text-[13px]" style={{ color: C.body }}>
                      ...
                      >
                        {pick(group.source.label)}
                      </a>
                    </p>
                  )}
                  {otherAudiences(group, aud).length > 0 && (
                    <div className="mt-4 border-t pt-3.5" style={{ borderColor: C.line }}>
                      <p className="text-[13px] font-bold" style={{ color: C.cocoa }}>
                        {pick(SPURDU.alsoFor)}
                      </p>
                      <div className="mt-2 flex flex-wrap gap-2">
                        {otherAudiences(group, aud).map((other) => {
                          const meta = AUDIENCES.find((a) => a.id === other)!
                          return (
                            <button
                              key={other}
                              type="button"
                              onClick={() => setAud(other)}
                              className="bofs-focus rounded-[11px] px-3 py-2 text-[13px] font-bold"
...
function Chevron({ open }: { open: boolean }) {
  return (
    ...
    >
      <path d="M3.5 6 L8 10.5 L12.5 6" stroke={C.clay} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  )
}
