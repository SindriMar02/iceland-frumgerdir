/**
 * THE OWNER'S SIDE — the dashboard Guðlaug would run. Deliberately a separate
 ...
 *
 * In Icelandic, because its audience is Guðlaug.
 * Declares its OWN @font-face — a lazy route inherits nothing from Page.tsx.
 * (Lineage: mysticlight/Dashboard.tsx, re-voiced.)
 */
...
const PANEL = '#FFFFFF'
const INK = '#22303A'
const MUTE = 'rgba(34,48,58,.66)'
const HAIR = 'rgba(34,48,58,.16)'
const RIVER = '#3E7A9E'
const DECLINE = '#B5563F'
...
export default function LaxfossDashboard() {
  ...
  return (
    <div style={{ minHeight: '100svh', background: FROST, color: INK, fontFamily: SANS, fontWeight: 400 }}>
      <style>{`
        @font-face { font-family: 'General Sans'; src: url('${import.meta.env.BASE_URL}laxfoss/fonts/GeneralSans-Regular.woff2') format('woff2'); font-weight: 400; font-display: swap; }
        @font-face { font-family: 'General Sans'; src: url('${import.meta.env.BASE_URL}laxfoss/fonts/GeneralSans-Medium.woff2') format('woff2'); font-weight: 500; font-display: swap; }
        .lxd-btn { font: inherit; font-size: 13px; font-weight: 500; cursor: pointer;
          border-radius: 3px; padding: 8px 14px; border: 1px solid ${HAIR};
          background: none; color: ${INK}; }
        .lxd-btn:hover { border-color: ${RIVER}; }
        .lxd-btn.is-confirm { background: ${RIVER}; color: #fff; border-color: ${RIVER}; }
        .lxd-btn.is-decline { color: ${DECLINE}; }
        .lxd-btn.is-decline:hover { border-color: ${DECLINE}; }
        .lxd-tab { font: inherit; font-size: 13px; cursor: pointer; background: none;
          border: 0; color: ${MUTE}; padding: 6px 0; border-bottom: 1px solid transparent; }
        .lxd-tab.is-on { color: ${INK}; border-bottom-color: ${RIVER}; }
        .lxd-focus :focus-visible { outline: 2px solid ${RIVER}; outline-offset: 2px; }
        .lxd-focus a { color: ${INK}; }
      `}</style>
      <div className="lxd-focus" style={{ maxWidth: 880, margin: '0 auto', padding: '40px 20px 80px' }}>
        <header style={{ display: 'flex', alignItems: 'baseline', gap: 18, flexWrap: 'wrap' }}>
          ...
          <Link to="/preview/laxfoss" style={{ marginLeft: 'auto', color: MUTE, fontSize: 13 }}>
            Aftur á gestasíðuna
          </Link>
        </header>
        ...
        <nav style={{ display: 'flex', gap: 24, margin: '36px 0 0', borderBottom: `1px solid ${HAIR}`, paddingBottom: 10 }} aria-label="Síur">
          <button type="button" className={`lxd-tab ${tab === 'nyjar' ? 'is-on' : ''}`} onClick={() => setTab('nyjar')}>
            Nýjar beiðnir {pending.length > 0 && `(${pending.length})`}
          </button>
          <button type="button" className={`lxd-tab ${tab === 'allar' ? 'is-on' : ''}`} onClick={() => setTab('allar')}>
            Allar ({rows.length})
          </button>
          <button type="button" className="lxd-tab" style={{ marginLeft: 'auto' }} onClick={() => demo.reset()}>
            Hreinsa sýnigögn
          </button>
        </nav>
        ...
        {shown.length === 0 ? (
          <div style={{ padding: '56px 0', color: MUTE, fontSize: 15, lineHeight: 1.6 }}>
            ...
            <p style={{ margin: '10px 0 0' }}>
              Farðu á <Link to="/preview/laxfoss">gestasíðuna</Link>,
              ...
              <li key={b.id} style={{
                ...
                {b.status === 'REQUESTED' && (
                  <div style={{ display: 'flex', gap: 10 }}>
                    <button type="button" className="lxd-btn is-confirm" onClick={() => demo.setStatus(b.id, 'CONFIRMED')}>
                      Staðfesta
                    </button>
                    <button type="button" className="lxd-btn is-decline" onClick={() => demo.setStatus(b.id, 'DECLINED')}>
                      Hafna
                    </button>
                  </div>
                )}
