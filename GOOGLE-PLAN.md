# Google sýnileiki: einfalt plan

Tvö skref hjá Katrínu, fjögur hjá Sindra. Restin gerist án hennar.

---

## Katrín

**1. Svaraðu tveimur spurningum** (2 mínútur)
- Er sýningarrýmið á Katrínartúni 4, eða annars staðar?
- Hvaða daga er opið? Vefurinn segir núna bara „11:00–17:00" og engan dag,
  því enginn hefur staðfest hverjir þeir eru. Rangur opnunartími er verri en
  enginn: fólk mætir að læstum dyrum og skrifar umsögn um það.

**2. Taktu upp tveggja mínútna myndband** (20 mínútur, Sindri kemur)

Google staðfestir fyrirtæki með einu samfelldu myndbandi úr símanum. Sindri
segir þér hvert á að beina símanum. Þú þarft ekkert að undirbúa. Þetta er
eina skrefið sem enginn getur gert fyrir þig, því það þarf að sýna að þú
ráðir yfir rýminu.

**Það er allt og sumt.** Umsagnir koma seinna og öðruvísi, sjá hér að neðan.

---

## Sindri

**1. Athuga hvað er þegar til** (5 mín, gerist fyrst)
Leita að henni á Google Maps. Þrjár mögulegar niðurstöður:
ekkert til (stofna nýtt) · óskráð færsla (gera kröfu, algengast) ·
einhver annar búinn að skrá (biðja um aðgang, 3–7 daga bið).

**2. Fara á staðinn einu sinni, án hennar** (30 mín)
- Sést merkingin í glugganum frá gangstétt?
- Sést húsnúmerið?
- Er sameiginlegur inngangur á undan hennar dyrum?
- **Taka 15 myndir af rýminu.** Myndirnar sem við eigum eru af vefnum hennar
  og gætu verið margra ára gamlar. Nýjar myndir eru það verðmætasta sem fer
  inn á Google-síðuna og enginn annar getur náð í þær.

**3. Myndbandið með henni** (20 mín, má vera sama heimsókn)
Út á gangstétt fyrst: götuskilti, húsnúmer, húsin á móti. Svo að glugganum,
halda 3 sekúndur á merkingunni. Svo opna hurðina á meðan síminn er í gangi.
Svo inn: eldhúsinnréttingin, steinsýnishornin, efnisskúffurnar. Ein taka, ekki
stoppa. Ef það mistekst segir Google hvers vegna og það má taka upp aftur.

**4. Fylla út og laga, án hennar** (2 klst, dreift)
- Google-síðan: flokkur „Innanhússarkitekt" sem aðal, „Kitchen furniture store"
  og „Bathroom supply store" sem auka. Sími, vefur, lýsing, myndirnar.
- Laga gömlu skráningarnar: **ja.is** (bæta FHI við starfsheitið),
  **yelu.is** (rangt heimilisfang, Skipasund 74),
  **starofservice.is** (tvær færslur undir „Studio Ísfeld").
- Sækja um prófíl á **honnunarmidstod.is** (hún er þegar í félagatali FHI).
- Arrital er þegar búið að skrá hana, sjá að neðan. Ekkert að senda.

---

## Umsagnir: ekki herferð, heldur venja

Fyrsta útgáfan af þessu plani bað Katrínu um að senda beiðni á tíu fyrri
viðskiptavini. Það er of mikið, og þrennt mælir gegn því:

- **Heimili eru einkamál.** Opinber umsögn þýðir að viðkomandi tengir nafnið
  sitt við sitt eigið heimili og þá staðreynd að hann réð hönnuð. Sumir vilja
  það alls ekki, og beiðnin setur þá í óþægilega stöðu. Þetta á ekki við um
  bakarí eða bílasölu, en það á við hér.
- **Gömul verkefni eru vandræðaleg.** Að skrifa einhverjum frá 2019 og biðja
  um greiða er annað en að spyrja þann sem er nýbúinn að flytja inn.
- **Tíu umsagnir á einni viku á glænýrri síðu er mynstur sem Google síar.**
  Jöfn dreifing er bæði kurteisari og tæknilega öruggari.

**Í staðinn, í þessari röð:**

1. **Byrja á fyrirtækjunum, ekki heimilunum.** Sex af verkefnunum hennar eru
   rekstur: Freyja gistiheimili, Freyja lúxusíbúð, Svala Apartments,
   Sólvallagata, Old Charm og Tannlæknastofan Garðatorgi. Að biðja fyrirtæki um
   umsögn er venjuleg viðskiptakurteisi, ekki greiði, og þau hafa sjálf hag af
   sýnileikanum. Tvær eða þrjár af þessum duga til að byrja.
2. **Gera það að hluta af verklokum héðan í frá.** Besti tíminn til að biðja um
   umsögn er daginn sem verkefni klárast og fólk er ánægt. Þá er þetta ekki
   verkefni á lista heldur venja, og hún skilar jöfnum straumi sem er einmitt
   það sem Google vill sjá.
3. **Aldrei senda á heimilisviðskiptavin nema sambandið sé hlýtt og nýlegt.**

## Það sem þarf engan greiða

Þetta er sterkara en tíu umsagnir og enginn þarf að segja já:

- **Hús og híbýli.** Verk hennar hafa birst þar oftar en einu sinni, og viðtöl
  við hana hafa verið birt. Þetta stendur á hennar eigin vef en hvergi á nýja
  vefnum. Það ætti að vera þar, og það er nákvæmlega sú tegund staðfestingar
  sem bæði Google og gervigreind taka mark á.
- **Miðstöð hönnunar og arkitektúrs.** Hún er þegar í félagatali FHI, svo þetta
  er beiðni en ekki umsókn.
- **Arrital: þegar komið.** Söluaðilaskrá Arrital (arrital.com/store-locator)
  skráir þegar einn íslenskan söluaðila og það er hún:
  „ICELAND | Reykjavík — Hönnunar Studio, Katrínartún 4, 105 Reykjavík,
  +354 663 3414, katrinisfeld.is". Tengillinn er til staðar, ekkert að gera.
  Þetta staðfestir líka heimilisfangið frá þriðja óháða aðila.
- **Altamarea: ekkert að sækja um.** Þeir reka enga söluaðilaskrá, „Showroom"
  hjá þeim er þeirra eigið sýningarrými á Ítalíu. Það eina sem væri hægt að
  spyrja um er umfjöllun í News/Focus, sem er kynningarmál með lélegar líkur,
  ekki leitarvélavinna. Sleppa því í bili.

---

## Tilbúinn texti fyrir umsagnabeiðni (fyrir fyrirtækjaviðskiptavini)

> Sæl og blessuð,
>
> Ég er að setja upp Google-síðu fyrir stúdíóið núna og er að safna nokkrum
> umsögnum. Ef þið væruð til í að skrifa tvær línur um samstarfið væri ég
> þakklát. Það tekur eina mínútu:
>
> [hlekkur]
>
> Ég set ykkur að sjálfsögðu líka inn í verkefnaskrána hjá mér.
>
> Kveðja,
> Katrín

---

## Röðun

Vika 1: spurningarnar tvær, athuga Maps, fara á staðinn og mynda.
Vika 2: myndbandið, fylla út síðuna.
Vika 3: gömlu skráningarnar og Hús og híbýli inn á vefinn.
Umsagnir: tvær eða þrjár frá fyrirtækjum þegar síðan er komin upp, svo jafnt
og þétt eftir því sem verkefni klárast.

**Heildartími Katrínar: um hálftími.**
