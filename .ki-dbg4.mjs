...
const d=await page.evaluate(()=>{
  ...
  return {sy:Math.round(sy), markCentreY:Math.round(sy+mark.top+mark.height/2), markX:Math.round(mark.left+mark.width/2),
    burger:(()=>{const b=document.querySelector('.ki-burger');const r=b.getBoundingClientRect();return {disp:getComputedStyle(b).display,on:b.dataset.kiOn,color:getComputedStyle(b).color,r:[Math.round(r.left),Math.round(r.top),Math.round(r.right),Math.round(r.bottom)]}})(), cond:document.querySelector('.ki-nav').dataset.kiCondensed, chromeCount:document.querySelectorAll('[data-ki-chrome]').length, inRoot:document.querySelectorAll('.ki-root [data-ki-chrome]').length, burgerAttr:document.querySelector('.ki-burger').getAttribute('data-ki-chrome'), navInRoot:!!document.querySelector('.ki-root .ki-nav'), bands:[]}})
