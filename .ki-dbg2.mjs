...
const d=await page.evaluate(()=>{
  ...
  return {sy:Math.round(sy), markCentreY:Math.round(sy+mark.top+mark.height/2), markX:Math.round(mark.left+mark.width/2),
    tone:document.querySelector('.ki-nav').dataset.kiTone, dbg:document.querySelector('.ki-nav').dataset.kiDbg, on:document.querySelector('.ki-nav-mark').dataset.kiOn, bands:out}})
