...
const page=await br.newPage(); page.on('pageerror',e=>console.log('PAGEERROR',e.message)); page.on('console',m=>{if(m.type()==='error')console.log('CONSOLE',m.text().slice(0,160))}); await page.setViewport({width:1600,height:1000})
...
for (const [idx,label] of [[0,'strata'],[1,'gate']]) {
  for (const p of [0,0.3,0.55,0.7,0.85,1]) {
    ...
    const d=await page.evaluate(i=>{
      ...
      const root=deep.closest('.parallax')
      const plates=[...root.querySelectorAll('[data-parallax-plate]')].map(e=>Math.round(new DOMMatrix(getComputedStyle(e).transform).m42))
      const S=[...deep.querySelectorAll('[data-parallax-stagger]')].slice(0,2).map(e=>(e.getAttribute('style')||'-').replace(/translate: none; rotate: none; scale: none; ?/,'').slice(0,40))
      return {S,plates,band:root.dataset.kiBand}}, idx)
    console.log(label+" p="+p+"  plates "+d.plates.join(",")+"  band "+d.band+"  stag "+d.S.join(" | "))
  }
}
