...
for (const [name,y] of Object.entries(spots)) {
  ...
  const st=await page.evaluate(()=>[...document.querySelectorAll('.ki-stratum')].map(e=>{const r=e.getBoundingClientRect();return Math.round(getComputedStyle(e).opacity*100)+'%@'+Math.round(r.left)+'w'+Math.round(r.width)+'h'+Math.round(r.height)}))
  console.log(name, st.join(' '))
}
